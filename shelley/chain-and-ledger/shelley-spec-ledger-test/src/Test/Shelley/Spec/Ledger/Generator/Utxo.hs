...
import Cardano.Ledger.Val(Val(..),minus,vsplit)
import Debug.Trace
...
genTx
  ...
  (utxoSt@(UTxOState utxo _ _ _), dpState) =
    do
      ...
      -------------------------------------------------------------------------
      let spendingBalance = spendingBalanceUtxo  <> (inject $ (sum (snd <$> wdrls)) - deposits + refunds)
...
instance Era era => Eq (Delta era) where
  a == b =
    dfees a == dfees b
...
deltaZero :: (Era era, Mock (Crypto era)) => Coin -> Coin -> Addr era -> Delta era
...
genNextDelta
  ...
  delta@(Delta dfees extraInputs extraWitnesses change _ _) =
    ...
    where
      -- deltaChange :: Era era => (v -> v) -> TxOut c v -> TxOut c v
...
applyDelta :: (Era era, Mock (Crypto era)) =>
 [KeyPair 'Witness era] ->
 Map (ScriptHash era) (MultiSig era) ->
 KeySpace era ->
 Tx era -> Delta era -> Tx era
applyDelta
  neededKeys
  neededScripts
...
mkTxWits ::
  (Era era, Mock (Crypto era),c ~ Crypto era) =>
...
calcOutputsFromBalance balance_ addrs fee =
  ...
  where
    ...
    balanceAfterFee = minus balance_ (inject fee)
...
genInputs ::
  ...
  UTxO era ->
  Gen ([TxIn era], v, ([KeyPair 'Witness c], [(MultiSig c, MultiSig c)]))
...
genWithdrawals ::
  (HasCallStack, CryptoClass.Crypto c) =>
  Constants ->
  Map (ScriptHash c) (MultiSig c, MultiSig c) ->
  Map (KeyHash 'Staking c) (KeyPair 'Staking c) ->
